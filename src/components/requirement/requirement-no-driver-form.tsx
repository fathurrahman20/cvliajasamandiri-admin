...
import EditRequirementNoDriver from "./modal-add-edit-requirement";
import AddRequirementNoDriver from "./modal-add-edit-requirement";
import DeleteRequirementNoDriver from "./dialog-delete-requirement-no-driver";

interface RequirementNoDriverTableProps {
  data: RequirementNoDriverDetailType[] | undefined;
}

export default function RequirementNoDriverTable({
  data,
}: RequirementNoDriverTableProps) {
  return (
    <div>
      <div className="mt-4 sm:mt-0 sm:flex-none">
        <AddRequirementNoDriver />
      </div>
                ...
                <tr key={item.id}>
                  ...
                  <td className="relative py-4 pl-3 text-right text-sm font-medium">
                    <div className="flex flex-row gap-x-2">
                      <EditRequirementNoDriver initialData={item} />
                      <DeleteRequirementNoDriver id={item.id} />
                    </div>
                  </td>
                </tr>
              ))}
